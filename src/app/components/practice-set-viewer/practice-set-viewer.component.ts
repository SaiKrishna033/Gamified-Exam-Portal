...
@Component({
  selector: 'app-practice-set-viewer',
  templateUrl: './practice-set-viewer.component.html',
  styleUrls: ['./practice-set-viewer.component.scss'],
})
export class PracticeSetViewerComponent {
  constructor(
  ...
  getPracticeSetsPerPage() {
    ...
    if (this.selected_subject_code == '') {
      ...
      this.http
        .get(
          this.kuheduService.baseUrl +
            `question/fetch-all-practice-sets-using-pagination?resultsPerPage=4&page=${this.page}`,
          { headers }
        )
        .subscribe((apidata: any) => {
          if (
            apidata.data.practiceSets.length > 0 &&
            apidata.data.practiceSets[apidata.data.practiceSets.length - 1]
              ._id != this.lastInSet
          ) {
            this.instructionData = apidata.data.practiceSets;
            this.lastInSet =
              apidata.data.practiceSets[
                apidata.data.practiceSets.length - 1
              ]._id;
      ...
      this.http
        .get(
          this.kuheduService.baseUrl +
            `question/fetch-all-practice-sets-using-pagination?resultsPerPage=4&page=${this.page}&subject_code=${this.selected_subject_code}`,
          { headers }
        )
        .subscribe((apidata: any) => {
          if (
  ...
  selectUnselectSubject(subCode: string) {
    this.selected_subject_code =
      this.selected_subject_code == subCode ? '' : subCode;
    this.page = 1;
    this.lastInSet = '';
    this.reached_last_page = false;
    ...
    if (this.selected_subject_code != '') {
      this.toast.success({
        detail: 'INFO',
        summary:
          'You have set a new filter on ' +
          this.getSubjectClass(this.selected_subject_code).toUpperCase(),
        duration: 4000,
